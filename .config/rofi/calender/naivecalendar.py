...
def set_locale_n_week_day_names(arg_locale, user_locale, day_format, first_day_week, day_abbr_lenght):
    ...
    # Custom Vietnamese day names (maps day number to short name)
    custom_day_names = {
        0: 'Sun',   # Chủ Nhật (Sunday)
        1: 'Mon',  # Thứ Hai (Monday)
        2: 'Tue',   # Thứ Ba (Tuesday)
        3: 'Wed',   # Thứ Tư (Wednesday)
        4: 'Thu',  # Thứ Năm (Thursday)
        5: 'Fri',  # Thứ Sáu (Friday)
        6: 'Sat'   # Thứ Bảy (Saturday)
    }

    def get_loc_day(day_num, lenght):
        """return locale day names truncated at lenght and titlized"""
        # Check if Vietnamese locale is being used
        current_locale = locale.getlocale()[0]
        if current_locale and 'vi_VN' in current_locale:
            # Use custom Vietnamese names
            day_name = custom_day_names.get(day_num, '')[:lenght] if lenght > 0 else custom_day_names.get(day_num, '')
        else:
            # Use default locale behavior for other languages
            day_name = locale.nl_langinfo(locale.DAY_1 + day_num)[:lenght].title()
        
        # Wrap with Pango markup to set custom color and bold for day names
        # Color: #cc241d (red) - change this hex code to your preferred color
        return f'<span foreground="#1d2021" weight="bold">{day_name}</span>'
...
######################
try:
    DEFAULT_EDITOR = cfg_t['LOCALE']["DEFAULT_EDITOR"]
except KeyError:
    DEFAULT_EDITOR = "code"  # fallback to vscode if not configured
...
################################
SYM_NEXT_MONTH = [f'<span foreground="#1d2021">{s}</span>' for s in to_list(cfg_t['CONTROL']['SYM_NEXT_MONTH'])]
SYM_PREV_MONTH = [f'<span foreground="#1d2021">{s}</span>'for s in to_list(cfg_t['CONTROL']['SYM_PREV_MONTH'])]
SYM_NEXT_YEAR = [f'<span foreground="#1d2021">{s}</span>'for s in to_list(cfg_t['CONTROL']['SYM_NEXT_YEAR'])]
SYM_PREV_YEAR = [f'<span foreground="#1d2021">{s}</span>'for s in to_list(cfg_t['CONTROL']['SYM_PREV_YEAR'])]

# Shortcuts for popup windows
#############################
SYM_SHOW_EVENTS = [f'<span foreground="#1d2021">{s}</span>'for s in to_list(cfg_t['SHORTCUTS']['SYM_SHOW_EVENTS'])]
...
SYM_SHOW_MENU = [f'<span foreground="#1d2021">{s}</span>'for s in to_list(cfg_t['SHORTCUTS']['SYM_SHOW_MENU'])]
...
def update_rofi(date, is_first_loop):
    ...
    # Enable Pango markup for all rows
    ...
    if not ROW_DAY_NAMES == EMPTY:
        ...
        # Don't mark as urgent or active - will be styled by row position in CSS
        # print(f"\0urgent\x1f{week_sym_row}\n")
...
def get_month_events(date):
    ...
    # Use current directory for events
    event_folder = Path.cwd() / "calendar_event"
    
    if not event_folder.exists():
        return []
    
    # Pattern: event_YYYY-MM-*.txt
    file_pattern = f"event_{date.year}-{date.month:02d}-*.txt"
    events_paths = list(event_folder.glob(file_pattern))
...
def get_month_events_ind(date):
    ...
    # Pattern: event_YYYY-MM-DD.txt
    pattern = r'event_\d{4}-\d{2}-(\d{2})\.txt'
    
    # Extract day numbers from filenames
    days = []
    for f in events_paths:
...
@open_n_reload_rofi
def show_day_menu(day_sym, date, editor=None):
    """Show menu with 3 options when clicking on a day"""
    
    # Use DEFAULT_EDITOR from config if not specified
    if editor is None or editor == "xdg-open":
    ...
    date_obj = datetime.date(date.year, date.month, day_ind)
    
    output = rofi_popup(f"Day {day_ind} - {date_obj.strftime('%B %Y')}", menu, nb_lines=3, width='15em')
    
    # Get event path in current directory
    event_folder = Path.cwd() / "calendar_event"
    event_path = event_folder / date_obj.strftime("event_%Y-%m-%d.txt")
    ...
    if "Write" in output:
        ...
        cmd = (*editor.split(' '), str(event_path))
        subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        
    elif "Delete" in output:
        if os.path.isfile(event_path):
            os.remove(event_path)
            rofi_popup("Success", "Event deleted!", nb_lines=1, width='15em')
        else:
            rofi_popup("Info", "No event found for this day", nb_lines=1, width='20em')
        ...
        if os.path.isfile(event_path):
            with open(event_path, 'r') as f:
                content = f.read()
            if content.strip():
                # Reduced size: 6 lines and 25em width instead of 10 lines and 40em
                rofi_popup(f"Event - {date_obj.strftime('%Y-%m-%d')}", content, nb_lines=6, width='25em')
            else:
                rofi_popup("Info", "Event file is empty", nb_lines=1, width='20em')
        else:
            rofi_popup("Info", "No event found for this day", nb_lines=1, width='20em')
...
def open_event(day_sym, date, editor):
    ...
    # Call show_day_menu instead of directly opening event
    show_day_menu(day_sym, date, editor)
